        ...
        <div className="flex gap-2">
          {MOODS.map(m => (
            <button key={m.value} onClick={()=>onMoodChange(m.value)}
              ...
              style={{
                background: mood===m.value ? '#10b98122' : 'var(--color-surface)',
                border: `1px solid ${mood===m.value ? 'var(--color-accent)' : 'var(--color-border)'}`
              }}>
              {m.emoji}
            </button>
          ))}
        ...
        <div className="flex gap-2">
          {[1,2,3,4,5].map(n => (
            <button key={n} onClick={()=>onEnergyChange(n)}
              ...
              style={{
                background: feltEnergy===n ? 'var(--color-accent)' : 'var(--color-surface)',
                color: feltEnergy===n ? 'var(--color-bg)' : 'var(--color-muted)',
                border: `1px solid ${feltEnergy===n ? 'var(--color-accent)' : 'var(--color-border)'}`
              }}>
              {n}
            </button>
          ))}
